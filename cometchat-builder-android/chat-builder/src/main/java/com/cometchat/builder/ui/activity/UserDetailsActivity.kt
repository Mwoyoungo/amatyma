...
import com.cometchat.builder.CometChatBuilderSettings.CallFeatures.VoiceAndVideoCalling.ONEONONEVIDEOCALLING
import com.cometchat.builder.CometChatBuilderSettings.CallFeatures.VoiceAndVideoCalling.ONEONONEVOICECALLING
import com.cometchat.builder.CometChatBuilderSettings.ChatFeatures.CoreMessagingExperience.USERANDFRIENDSPRESENCE
...
import com.cometchat.builder.databinding.BuilderActivityUserDetailsBinding
import com.cometchat.builder.utils.AppUtils.customToast
import com.cometchat.builder.viewmodels.UserDetailsViewModel
import com.google.gson.Gson

class UserDetailsActivity : AppCompatActivity() {
    private var binding: BuilderActivityUserDetailsBinding? = null
    private var alertDialog: CometChatConfirmDialog? = null
    private var viewModel: UserDetailsViewModel? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = BuilderActivityUserDetailsBinding.inflate(
            layoutInflater
        )
    ...
    private fun initViewModel() {
        ...
        viewModel!!.setUser(
            Gson().fromJson(
                intent.getStringExtra(getString(R.string.builder_user)), User::class.java
            )
        )
        viewModel!!.setBaseMessage(
            Gson().fromJson(
                intent.getStringExtra(getString(R.string.builder_base_message)), BaseMessage::class.java
            )
        )
    ...
    private fun initClickListeners() {
        ...
        binding!!.tvBlock.setOnClickListener { v: View? ->
            if (binding!!.tvBlock.text.toString() == getString(R.string.builder_block)) {
                blockUser()
            } else {
                unblockUser()
            }
    ...
    private fun daleteChatStateObserver(): Observer<Boolean> {
        return Observer { isChatDeleted: Boolean ->
            if (isChatDeleted) {
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_delete_chat_success), CometChatTheme.getColorBlack(
                        this
                    )
                )
                finish()
                ...
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_delete_chat_error), CometChatTheme.getErrorColor(
                        this
                    )
                )
            }
    ...
    private val isUserBlockedByMe: Observer<Boolean>
        get() = Observer { isUserBlockedByMe: Boolean ->
            if (isUserBlockedByMe) {
                binding!!.tvBlock.text = getString(R.string.builder_unblock)
            } else {
                binding!!.tvBlock.text = getString(R.string.builder_block)
            }
        }
    ...
    private fun blockUserStateObserver(): Observer<Boolean> {
        return Observer { isUserBlocked: Boolean ->
            if (isUserBlocked) {
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_block_user_success), CometChatTheme.getColorBlack(
                        this
                    )
                )
            } else {
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_block_user_error), CometChatTheme.getErrorColor(
                        this
                    )
                )
            }
    ...
    private fun unblockUserStateObserver(): Observer<Boolean> {
        return Observer { isUserUnblocked: Boolean ->
            if (isUserUnblocked) {
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_unblock_user_success), CometChatTheme.getColorBlack(
                        this
                    )
                )
            } else {
                alertDialog!!.dismiss()
                customToast(
                    this, getString(R.string.builder_unblock_user_error), CometChatTheme.getErrorColor(
                        this
                    )
                )
            }
            ...
            if (user != null) {
                ...
                if (!Utils.isBlocked(user)) {
                    ...
                    if (user.isBlockedByMe) {
                        ...
                        binding!!.tvInfoMessage.text = String.format("%s %s", getString(R.string.builder_you_have_blocked_this_user), user.name)
                    } else {
                        binding!!.infoMessage.visibility = View.VISIBLE
                        binding!!.tvInfoMessage.text = String.format("%s %s", user.name, getString(R.string.builder_has_blocked_you))
                    }
                }

                if (!ONEONONEVIDEOCALLING) binding!!.cardVideoCall.visibility = View.GONE

                if (!ONEONONEVOICECALLING) binding!!.cardVoiceCall.visibility = View.GONE

                if (!USERANDFRIENDSPRESENCE) binding!!.tvSubtitle.visibility = View.GONE
    ...
    private fun blockUser() {
        showCometChatConfirmDialog(
            ...
            getString(R.string.builder_block_user_title),
            getString(R.string.builder_block_user_subtitle),
            getString(R.string.builder_block_user_positive_button),
            getString(R.string.builder_block_user_negative_button),
            { view: View? ->
                alertDialog!!.hidePositiveButtonProgressBar(false)
                viewModel!!.blockUser()
            },
    ...
    private fun unblockUser() {
        showCometChatConfirmDialog(
            ...
            getString(R.string.builder_unblock_user_title),
            getString(R.string.builder_unblock_user_subtitle),
            getString(R.string.builder_unblock_user_positive_button),
            getString(R.string.builder_unblock_user_negative_button),
            { view: View? ->
                alertDialog!!.hidePositiveButtonProgressBar(false)
