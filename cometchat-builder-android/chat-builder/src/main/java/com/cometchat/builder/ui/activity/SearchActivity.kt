...
import com.cometchat.builder.databinding.BuilderActivitySearchBinding
...
class SearchActivity : AppCompatActivity() {
    ...
    private lateinit var binding: BuilderActivitySearchBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = BuilderActivitySearchBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val userJson = intent.getStringExtra(getString(R.string.builder_user))
        val groupJson = intent.getStringExtra(getString(R.string.builder_group))
        try {
            if (userJson != null) {
    ...
    private fun handleMessagesScreenNavigation(
        ...
        if (isSameChat && parentMessage == null) {
            val resultIntent = Intent().apply {
                ...
                baseMessage?.let {
                    putExtra(getString(R.string.builder_go_to_message), it.rawMessage.toString())
                }
            }
            ...
            if (parentMessage != null) {
                ...
                val resultIntent = Intent().apply {
                    ...
                    targetUser?.let {
                        putExtra(getString(R.string.builder_user), it.toJson().toString())
                    }
                    targetGroup?.let {
                        putExtra(getString(R.string.builder_group), Gson().toJson(it))
                    }
                    baseMessage?.let {
                        putExtra(getString(R.string.builder_go_to_message), baseMessage.rawMessage.toString())
                    }
                    parentMessage?.let {
    ...
    private fun handleConversationsScreenNavigation(
        ...
        targetUser?.let {
            intent.putExtra(getString(R.string.builder_user), it.toJson().toString())
        }
        targetGroup?.let {
            intent.putExtra(getString(R.string.builder_group), Gson().toJson(it))
        }
        baseMessage?.let {
            intent.putExtra(getString(R.string.builder_go_to_message), it.rawMessage.toString())
        }
