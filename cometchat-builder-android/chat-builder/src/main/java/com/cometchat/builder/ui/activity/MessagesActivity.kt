...
import com.cometchat.builder.CometChatBuilderSettings.ChatFeatures.DeeperUserEngagement.GROUPINFO
import com.cometchat.builder.CometChatBuilderSettings.ChatFeatures.DeeperUserEngagement.USERINFO
...
import com.cometchat.builder.databinding.BuilderActivityMessagesBinding
import com.cometchat.builder.databinding.BuilderOverflowMenuLayoutBinding
...
class MessagesActivity : AppCompatActivity() {
    ...
    private lateinit var binding: BuilderActivityMessagesBinding
    ...
    private fun handleDifferentChatNavigation(data: Intent) {
        val selectedUserJson = data.getStringExtra(getString(R.string.builder_user))
        val selectedGroupJson = data.getStringExtra(getString(R.string.builder_group))
        val goToMessageJson = data.getStringExtra(getString(R.string.builder_go_to_message))
        ...
        if (parentMessageJson != null) {
            val intent = Intent(this, ThreadMessageActivity::class.java).apply {
                ...
                selectedUserJson?.let { putExtra(getString(R.string.builder_user), it) }
                selectedGroupJson?.let { putExtra(getString(R.string.builder_group), it) }
            }
            ...
            val intent = Intent(this, MessagesActivity::class.java).apply {
                selectedUserJson?.let { putExtra(getString(R.string.builder_user), it) }
                selectedGroupJson?.let { putExtra(getString(R.string.builder_group), it) }
                goToMessageJson?.let { putExtra(getString(R.string.builder_go_to_message), it) }
            }
            startActivity(intent)
            finish()
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = BuilderActivityMessagesBinding.inflate(layoutInflater)
        ...
        group = Gson().fromJson(intent.getStringExtra(getString(R.string.builder_group)), Group::class.java)
        try {
            val rawGoToMessage = intent.getStringExtra(getString(R.string.builder_go_to_message))
        ...
        BuilderApplication.currentOpenChatId = if (group != null) group!!.guid else user?.uid
        ...
        binding.messageList.mentionsFormatter.setOnMentionClick { context: Context, user: User? ->
            ...
            intent.putExtra(context.getString(R.string.builder_user), Gson().toJson(user))
    ...
    private fun getHeaderMenuOptions(): List<CometChatPopupMenu.MenuItem> {
        ...
        if (CometChatBuilderSettings.ChatFeatures.CoreMessagingExperience.CONVERSATIONANDADVANCEDSEARCH) {
            options.add(
        ...
        if (CometChatBuilderSettings.ChatFeatures.AiUserCopilot.CONVERSATIONSUMMARY) {
            options.add(
                CometChatPopupMenu.MenuItem(
        ...
        if ((user != null && USERINFO) || (group != null && GROUPINFO)) {
            options.add(
                CometChatPopupMenu.MenuItem(
                    UIKitConstants.MessageHeaderMenuOptions.DETAILS,
                    getString(com.cometchat.chatuikit.R.string.cometchat_details),
                    AppCompatResources.getDrawable(this@MessagesActivity, R.drawable.builder_ic_info),
                    null
                ) {
                    openDetailScreen()
                }
    ...
    private fun setOverFlowMenu() {
        binding.messageHeader.setTrailingView { context: Context?, user: User?, group: Group? ->
            val linearLayout = LinearLayout(context)
            val overflowMenuLayoutBinding = BuilderOverflowMenuLayoutBinding.inflate(layoutInflater)
            overflowMenuLayoutBinding.ivMenu.setImageResource(R.drawable.builder_ic_info)
    ...
    private fun openDetailScreen(group: Group?) {
        ...
        if (user != null) {
            intent = Intent(this, UserDetailsActivity::class.java)
            intent.putExtra(getString(R.string.builder_user), Gson().toJson(user))
            intent.putExtra(getString(R.string.builder_base_message), Gson().toJson(binding.messageList.viewModel.lastMessage))
        } else if (group != null) {
            intent = Intent(this, GroupDetailsActivity::class.java)
            intent.putExtra(getString(R.string.builder_group), Gson().toJson(group))
            intent.putExtra(getString(R.string.builder_base_message), Gson().toJson(binding.messageList.viewModel.lastMessage))
    ...
    override fun onDestroy() {
        ...
        BuilderApplication.currentOpenChatId = null
    }
}
