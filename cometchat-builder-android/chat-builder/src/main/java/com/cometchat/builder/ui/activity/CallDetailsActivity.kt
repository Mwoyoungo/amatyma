...
import com.cometchat.builder.databinding.BuilderActivityCallDetailsBinding
import com.cometchat.builder.viewmodels.CallDetailsViewModel
...
import com.cometchat.builder.ui.adapters.CallDetailsTabFragmentAdapter
import com.cometchat.builder.ui.fragments.CallDetailsTabHistoryFragment
import com.cometchat.builder.ui.fragments.CallDetailsTabParticipantsFragment
import com.cometchat.builder.ui.fragments.CallDetailsTabRecordingFragment
...
class CallDetailsActivity : AppCompatActivity() {
    private lateinit var binding: BuilderActivityCallDetailsBinding
    private lateinit var viewModel: CallDetailsViewModel
    private lateinit var callLog: CallLog

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = BuilderActivityCallDetailsBinding.inflate(
            layoutInflater
        )
    ...
    private fun initTabFragment() {
        ...
        adapter.addFragment(
            CallDetailsTabParticipantsFragment(), getString(R.string.builder_call_details_participants), callLog
        )
        adapter.addFragment(
            CallDetailsTabRecordingFragment(), getString(R.string.builder_call_details_recordings), callLog
        )
        adapter.addFragment(
            CallDetailsTabHistoryFragment(), getString(R.string.builder_call_details_history), callLog
        )
    ...
    private fun updateInfoView() {
        ...
        if (callLog.type == CometChatCallsConstants.CALL_TYPE_AUDIO || callLog.type == CometChatCallsConstants.CALL_TYPE_VIDEO || callLog.type == CometChatCallsConstants.CALL_TYPE_AUDIO_VIDEO) {
            if (isLoggedInUser) {
                binding.tvInfoTitle.setText(R.string.builder_call_outgoing)
                ...
                binding.tvInfoTitle.setText(R.string.builder_call_missed)
                binding.tvInfoTitle.setTextAppearance(
                    CometChatTheme.getTextAppearanceHeading4Medium(
                        this
                    )
                ...
                binding.tvInfoTitle.setText(R.string.builder_call_incoming)
                binding.tvInfoTitle.setTextAppearance(
                    CometChatTheme.getTextAppearanceHeading4Medium(
                        this
                    )
